...
LoginForm.addEventListener("submit", function (e) {
  ...
  const AUTH_API = "http://localhost:8081/auth/login";
...
function getCustomers(pageNo, rowsCount, sortBy) {
  const apiUrl = `http://localhost:8081/customer/getCustomers?pageNo=${pageNo}&rowsCount=${rowsCount}&sortBy=${sortBy}`;
...
addCustomerForm.addEventListener("submit", (e) => {
  ...
  const apiUrl = "http://localhost:8081/customer/create?SyncDb=false";
...
function deleteCust(event) {
  ...
  const apiUrl = `http://localhost:8081/customer/delete?email=${email}`;
...
function editRow(btn) {
  ...
  editCustomerForm.addEventListener("submit", function (e) {
    ...
    const authToken = localStorage.getItem("jwtToken");
    const apiUrl = `http://localhost:8081/customer/update/${personToupdate}`;
    fetch(apiUrl, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
...
function search() {
  ...
  const apiUrl = `http://localhost:8081/customer/searchBy?searchBy=${selectedValue}&searchQuery=${searchValue}`;
...
function sync() {
  let customersSync = [];
  const apiUrl = `http://localhost:8081/customer/sync`;
  ...
  fetch(apiUrl, {
    ...
    .then((data) => {
      ...
      customersSync.forEach((customer) => {
        ...
        const apiUrl = "http://localhost:8081/customer/create?SyncDb=true";
